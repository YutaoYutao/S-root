...
path = "/Users/zyt/Desktop/test-2nd/" #path where csvfiles are
setupFile <- read.csv("tiltingUvAplate.csv", sep = ",") #make a file explain the 8 plates.
timeStep <- 20 #minutes
ExpName <- "Direction"
...
require(ggplot2)
  ...
  Summary_main<- read.csv("reAngSummary_main.csv")
  #Angle vector
  p <- ggplot(Summary_main[grep("Con",Summary_main$Treatment),], aes(x=timeCategory*20/60, y=(reAng*-1), colour = Genotype))
  p  + geom_point(size = 0.1) + xlab("hours after start treatment") + ylab("response Root vectorAngle") + xlim(0,48) +geom_line(aes(group=Genotype))+
    geom_errorbar(aes(ymin= (reAng*-1)- SE_Ang, ymax=(reAng*-1)+SE_Ang, width=.1))

  #nodeDirectionAv
  p <- ggplot(Summary_main[grep("Na",Summary_main$Treatment),], aes(x=timeCategory*20/60, y=(DirAv*-1), colour = Genotype))
  p  + geom_point(size = 0.1) + xlab("hours after start treatment") + ylab("NaCl nodeDirectionAv") + xlim(0,24) +geom_line(aes(group=Genotype))+
    geom_errorbar(aes(ymin= (DirAv*-1)- SE_DirAv, ymax=(DirAv*-1)+SE_DirAv, width=.1))

  #Angle vector
  p <- ggplot(Summary_main[grep("Na",Summary_main$Treatment),], aes(x=timeCategory*20/60, y=(Ang*-1), colour = Genotype))
  p  + geom_point(size = 0.1) + xlab("hours after start treatment") + ylab("NaCl Root vectorAngle") + xlim(0,48) +geom_line(aes(group=Genotype))+
  ...
  p  + geom_point(size = 0.1) + xlab("hours after start treatment") + ylab("Control Root vectorAngle") + xlim(0,48) +geom_line(aes(group=Genotype))+
    geom_errorbar(aes(ymin= (Ang*-1)- SE_Ang, ymax=(Ang*-1)+SE_Ang, width=.1))

  #Elongation
  p <- ggplot(Summary_main[grep("Na",Summary_main$Treatment),], aes(x=timeCategory*20/60, y=GR, colour = Genotype))
  p  + geom_point(size = 0.1) + xlab("hours after start treatment") + ylab("NaCl Growth rate") + xlim(0,48) +geom_line(aes(group=Genotype))+
