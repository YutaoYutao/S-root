...
path = "~/Desktop/test1/" #path where csvfiles are
setupFile <- read.csv("test1setup.csv", sep = ",") #make a file explain the 8 plates.
timeStep <- 20 #minutes
ExpName <- "Direction"
View(setupFile)

RITEtoData <- function(path, setupFile, timeStep, ExpName){
  storewd <- getwd()
  setwd(path)
  ...
  for(i in 1:length(file.names)){
    ...
    DP <- t(as.data.frame(strsplit(as.character(file$image),"Pos"))) #split imagename
    colnames(DP) <- c("Date", "Plate") #Add column names
    file <- cbind(file, DP[,1:2]) #bind date to the current dataset
    file$Date <- strptime(file$Date, "%Y-%m-%d_%H-%M")
...
RITEtoData(path, setupFile, timeStep, ExpName)
...
Summary_main<- read.csv("rDirection_ElongationSummary_main.csv")
...
#ElongationToGraph <- function(Data_main, Summary_main, SavePath, timeResolution){
  ...
  for(i in 1:nlevels(Main_smooth$root)){
    ...
    PLOT <- paste(SavePath, levels(Main_smooth$root)[i], TreatmentName, GenotypeName,  sep="_")
    p <- ggplot(Data, aes(x=HAS, y=Dirdiff))
    p + geom_line() + geom_point(size = 0.5) + xlab("Hours from treatment")+ xlim(0,24)
...
#Direction
p <- ggplot(Summary_main[grep("Con",Summary_main$Treatment),], aes(x=timeCategory*20/60, y=(Dir*-1), colour = Genotype))
p  + geom_point(size = 0.1) + xlab("hours after start treatment") + ylab("Direction") + xlim(0,24) +geom_line(aes(group=Genotype))+
  geom_errorbar(aes(ymin= (Dir*-1)- SE_Dir, ymax=(Dir*-1)+SE_Dir, width=.1))

#nodeDirectionAv snrk
p <- ggplot(Summary_main[grep("NaCl",Summary_main$Treatment),], aes(x=timeCategory*20/60, y=(DirAv*-1), colour = Genotype))
p  + geom_point(size = 0.1) + xlab("hours after start treatment") + ylab("NaCl100mM RTD(Angle in Degree)") + xlim(0,24) +geom_line(aes(group=Genotype))+
  geom_errorbar(aes(ymin= (DirAv*-1)- SE_DirAv, ymax=(DirAv*-1)+SE_DirAv, width=.1))

#nodeDirectionAv Col0
p <- ggplot(Summary_main[grep("Col0",Summary_main$Genotype),], aes(x=timeCategory*20/60, y=(DirAv*-1), colour = Treatment))
p  + geom_point(size = 0.1) + xlab("hours after start treatment") + ylab("Col0 RTD(Angle in Degree)") + xlim(0,24) +geom_line(aes(group=Treatment))+
  geom_errorbar(aes(ymin= (DirAv*-1)- SE_DirAv, ymax=(DirAv*-1)+SE_DirAv, width=.1))

#nodeDirection10
p <- ggplot(Summary_main[grep("R",Summary_main$Treatment),], aes(x=timeCategory*20/60, y=(Dir10*-1), colour = Treatment))
p  + geom_point(size = 0.1) + xlab("hours after start treatment") + ylab("nodeDirection10") + xlim(0,50) +geom_line(aes(group=Treatment))+
  geom_errorbar(aes(ymin= (Dir10*-1)- SE_Dir10, ymax=(Dir10*-1)+SE_Dir10, width=.1))

#Vector Angle
p <- ggplot(Summary_main[grep("Con",Summary_main$Treatment),], aes(x=timeCategory*20/60, y=(Ang*-1), colour = Genotype))
p  + geom_point(size = 0.1) + xlab("hours after start treatment") + ylab("Control RVA(Angle in Degree)") + xlim(0,24) +geom_line(aes(group=Genotype))+
  geom_errorbar(aes(ymin= (Ang*-1)- SE_Ang, ymax=(Ang*-1)+SE_Ang, width=.1))

#Vector Angle Col0
p <- ggplot(Summary_main[grep("Col0",Summary_main$Genotype),], aes(x=timeCategory*20/60, y=(Ang*-1), colour = Treatment))
p  + geom_point(size = 0.1) + xlab("hours after start treatment") + ylab("Col0 RVA(Angle in Degree)") + xlim(0,24) +geom_line(aes(group=Treatment))+
  geom_errorbar(aes(ymin= (Ang*-1)- SE_Ang, ymax=(Ang*-1)+SE_Ang, width=.1))

#nodeDirAvDiff
p <- ggplot(Summary_main[grep("R",Summary_main$Treatment),], aes(x=timeCategory*20/60, y=(DirAvdiff), colour = Treatment))
p  + geom_point(size = 0.1) + xlab("hours after start treatment") + ylab("nodeDirAv Change") + xlim(0,50) +geom_line(aes(group=Treatment))+
  geom_errorbar(aes(ymin= (DirAvdiff)- SE_DirAvdiff, ymax=(DirAvdiff)+SE_DirAvdiff, width=.1))


#Direction difference
p <- ggplot(Summary_main[grep("R",Summary_main$Treatment),], aes(x=timeCategory*20/60, y=Dirdiff, colour = Treatment))
p  + geom_point(size = 0.1) + xlab("hours after start treatment") + ylab("Direction change (degrees)") + xlim(0,24) +geom_line(aes(group=Treatment))+
  geom_errorbar(aes(ymin= Dirdiff- SE_Dirdiff, ymax=Dirdiff+SE_Dirdiff, width=.1))

#Elongation
p <- ggplot(Summary_main[grep("NaC",Summary_main$Treatment),], aes(x=timeCategory*20/60, y=GR, colour = Genotype))
p  + geom_point(size = 0.1) + xlab("hours after start treatment") + ylab("NaCl100mM GrowthRate") + xlim(0,25) +geom_line(aes(group=Genotype))+
...
p  + geom_point(size = 0.1) + xlab("hours after start treatment") + ylab("Col0 GrowthRate") + xlim(0,25) +geom_line(aes(group=Treatment))+
  geom_errorbar(aes(ymin= GR- SE_GR, ymax=GR+SE_GR, width=.1))

#Boxplot phases
  p <- ggplot(subset(Main_Phases, Treatment == "Salt"), aes(x=Genotype, y = SP, fill = Genotype))
  p + geom_boxplot(notch = FALSE) + ylab("Quiescence Phase (hours)")
