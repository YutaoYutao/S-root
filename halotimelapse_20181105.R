#halotropism-timelapse
#RITEtoData: Transform Dataset
path = "/Users/zyt/Desktop/exp1/" #path where csvfiles are
setupFile <- read.csv("exp1plate.csv", sep = ",") #make a file explain the 8 plates.
timeStep <- 20 #minutes
ExpName <- "Direction"
...
#ElongationToGraph <- function(Data_main, Summary_main, SavePath, timeResolution){
  ...
  for(i in 1:nlevels(Main_smooth$root)){
    ...
    p + geom_line() + geom_point(size = 0.5) + xlab("Time")
...
require(ggplot2)
  ...
  #nodeDirectionAv
  p <- ggplot(Summary_main[grep("Con",Summary_main$Treatment),], aes(x=timeCategory*20/60, y=(DirAv), colour = Genotype))
  p  + geom_point(size = 0.1) + xlab("hours after start treatment") + ylab("Control DirectionAv") + xlim(0,24) +geom_line(aes(group=Genotype))+
    geom_errorbar(aes(ymin= (DirAv)- SE_DirAv, ymax=(DirAv)+SE_DirAv, width=.1))

  #nodeDirection10
  p <- ggplot(Summary_main[grep("Con",Summary_main$Treatment),], aes(x=timeCategory*20/60, y=(Dir10), colour = Genotype))
  p  + geom_point(size = 0.1) + xlab("hours after start treatment") + ylab("Control Direction10") + xlim(0,24) +geom_line(aes(group=Genotype))+
    geom_errorbar(aes(ymin= (Dir10)- SE_Dir10, ymax=(Dir10)+SE_Dir10, width=.1))

  #nodeDirection5
  p <- ggplot(Summary_main[grep("Con",Summary_main$Treatment),], aes(x=timeCategory*20/60, y=(Dir5), colour = Genotype))
  p  + geom_point(size = 0.1) + xlab("hours after start treatment") + ylab("Control Direction5") + xlim(0,24) +geom_line(aes(group=Genotype))+
    geom_errorbar(aes(ymin= (Dir5)- SE_Dir5, ymax=(Dir5)+SE_Dir5, width=.1))
  ...
  #Elongation
  p <- ggplot(Summary_main[grep("NaCl",Summary_main$Treatment),], aes(x=timeCategory*20/60, y=GR, colour = Genotype))
  p  + geom_point(size = 0.1) + xlab("hours after start treatment") + ylab("NaCl Growth rate") + xlim(2,24) +geom_line(aes(group=Genotype))+
